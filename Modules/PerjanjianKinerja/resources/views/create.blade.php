                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-select" id="atasan_id" name="atasan_id" required disabled>
                                        <option value="">-- Pilih pegawai terlebih dahulu --</option>
                                    </select>
                                    <div class="invalid-feedback">Atasan belum tersedia</div>
                                    <div class="form-text">
...
@push('scripts')
    <script>
        ...
        function loadAtasan(pegawaiId) {
            if (!pegawaiId) {
                $('#atasan_id').html('<option value="">-- Pilih pegawai terlebih dahulu --</option>').prop('disabled',
                true);
                $('#preview-atasan').text('-');
                $('#preview-atasan-detail').html('');
                return;
            ...
            $.ajax({
                ...
                type: 'GET',
                beforeSend: function() {
                    $('#atasan_id').html('<option value="">Loading...</option>').prop('disabled', true);
                },
                success: function(response) {
                    if (response.success && response.data) {
                        const atasan = response.data;
                        $('#atasan_id').html(
                            `<option value="${atasan.id}" data-nip="${atasan.nip}" data-jabatan="${atasan.jabatan}">${atasan.nama} - ${atasan.nip}</option>`
                        ).prop('disabled', false);
                        ...
                        $('#atasan_id').html('<option value="">Atasan tidak ditemukan</option>').prop(
                            'disabled', true);
                        Swal.fire({
                            icon: 'warning',
                            title: 'Perhatian',
                ...
                error: function() {
                    $('#atasan_id').html('<option value="">Error loading atasan</option>').prop('disabled',
                        true);
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
